...
PAD_OUTER_MAX = 30           # look→(전진/회전 or face+접근) 시도 최대 횟수. 18→30: 라이브에서 거의
                             # 다 갔는데 조기 포기→360 재탐색으로 낭비. area 기반 진전판정이 진짜 정체는
                             # 빠르게 걸러내므로 상한만 늘려 near-success가 완주하게. anchor goal-seek는
                             # VLM을 생략해 반복이 싸므로(~5s), 벽 통과 탐색 여유를 넉넉히 둡니다.
PAD_NAV_BUDGET_S = 240.0     # ★한 번의 visual_navigate_to_pad 벽시계 상한(초). 150→240: near-success
                             # 완주 여유(VLM 30~40s가 지배비용). 진짜 정체는 escapes 상한이 먼저 끊음. 라이브 확정:
...
PAD_PLACE_TRIES = 10         # C 확인 후 배송(delivered↑)될 때까지 place+전진을 반복할 최대 횟수. 4→10.
PLACE_CREEP_VX = 0.4         # place 전 '제대로' 다가가는 전진 속도. 약한 vx(0.2)는 램프업(~0.35s)에
                             # 다 죽어 제자리 → 시뮬이 안 움직임(라이브 확정). 0.4/1.0s로 실제 이동.
PLACE_CREEP_DUR = 1.0        # place 전진 한 번의 시간(초).
PLACE_STUCK_LIMIT = 3        # 전진이 '연속' 이만큼 막히면 중단(그때만 nav가 다른 각도로 재접근).
# --- 진전 판정·갇힘 탈출(왔다갔다 차단) — 진전은 '물리 이동'이 아니라 'pad가 커짐(가까워짐)'으로 잰다 ---
# 적대검증 확정: 순이동(net displacement)으로 진전을 재면 옆으로 드리프트만 해도 '진전'으로 인정돼
# 갇힘 포기가 영영 안 터지고, 방향을 매 프레임 angle 부호로 재계산하면 wedge 정면(angle≈0) 노이즈에
# 좌우로 튀어 왔다갔다가 난다. → 진전=blob 면적 증가(가까워짐), 탈출 방향=latch(물리적 막힘 때만 전환).
PAD_PROGRESS_GROWTH = 1.15   # pad blob 면적(EMA)이 직전 최대의 이 배를 넘어야 '가까워짐'(진전)으로
                             # 인정. 라이브 노이즈 대역(13k~15k≈15%)보다 크게 잡아 노이즈 스파이크를
                             # 진전으로 오인(→latch 해제 진동)하지 않게. EMA 평활과 병행.
PAD_AREA_EMA = 0.5           # pad 면적 EMA 계수(0.5=최근2샘플 평균급). 단발 노이즈로 진전/정체 오판 방지.
PAD_STUCK_LIMIT = 3          # 이 attempt 동안 안 가까워지면(갇힘) 커밋 벽따라 탈출.
PAD_MAX_ESCAPES = 5          # 탈출을 이만큼 해도 안 가까워지면 nav 포기 → recover/LLM 재시도. 3→5.
PAD_PLACE_RANGE_M = 1.20     # (로그 실측) place는 pad 중심 1.20m 이내여야 성공. 도착=area만으론 부족.
# --- 수확 pad 지향(랜덤 시작/멀리 있는 pad 발견) ---
PAD_HARVEST_SCAN_TURNS = 2   # 목표색 안 보일 때, 이만큼 제자리 재확인 후 수확 pad 쪽으로 지향 이동.
PAD_HARVEST_MAX_SEEK = 4     # 수확 pad 지향 이동 상한(무한 추격 방지).
...
async def _advance_or_detour(
    ...
    *,
    ...
    # ★win_side(detour_wins 이력) 오버라이드 제거(적대검증 확정): center-approach에선 goal_side
    #   보호가 죽어(|goal_turn|<=10<15) 이력이 호출부의 latch된 side를 무조건 덮어써 방향이
    #   뒤집혔다. 방향 일관성은 호출부(committed_side latch)가 책임진다 — 여기선 준 side를 그대로 쓴다.
...
async def _lateral_bypass(
    ...
    *,
    ...
    moved_any = False
    consec_stall = 0   # ★연속 미달 카운터: 단발 TimeoutError/노이즈 한 방으로 '물리적 막힘' 오판 방지.
    for _ in range(chunks):
        moved = await _strafe(ctx, side, duration_s=PAD_ADVANCE_DUR)
        p1 = await _get_pose(ctx)
        stalled_chunk = moved < STRAFE_MIN_M
        _trace_step(
            memory, action="bypass_strafe", pose=p1,
            actual_m=round(moved, 3), stall=stalled_chunk,
        )
        if stalled_chunk:
            consec_stall += 1
            if consec_stall >= 2:
                break  # 2연속 미달 = 진짜 옆도 막힘 -> 중단(호출부가 반대쪽 시도).
            continue   # 단발 미달(타임아웃/노이즈 가능) -> 한 번 더 시도.
        consec_stall = 0
        _bump_stat(memory, "path_len_m", moved)
        moved_any = True
...
def _nearest_harvest_anchor(
    memory: AgentMemory | None,
    held_color: str,
    pose: dict[str, float],
    *,
    max_stale_s: float = PAD_ANCHOR_STALE_S,
) -> tuple[str, dict[str, float], float, float] | None:
    """목표색을 뺀, 이번 런에 수확된 '다른' pad anchor 중 가장 가까운 것을
    (색, anchor, 거리, face_turn)로 반환. 없으면 None.

    ★쓰는 가정은 '패드가 한 구역에 모여 있다'는 관측 사실뿐 — B-C-D-E의 순서/기하는 안 쓴다
    (규칙 미제공). 순서를 몰라도 '가장 가까운 수확 pad 근처 = 목표 pad도 이웃'일 확률이 높다는
    약한(그래서 안전한) 전제만. anchor는 전부 이번 런 VLM 수확 산물 → 하드코딩 좌표 아님(랜덤 시작 안전).
    낡은/0가중 anchor는 제외(그동안 이동으로 어긋났을 수 있음).
    """
    if memory is None:
        return None
    now = time.monotonic()
    best: tuple[str, dict[str, float], float, float] | None = None
    for color, e in memory.pad_memory.items():
        if color == held_color or not isinstance(e, dict):
            continue
        a = e.get("anchor")
        if not a or float(a.get("w_sum", 0.0)) <= 0.0:
            continue
        if now - float(a.get("t", now)) > max_stale_s:
            continue
        dist, face = _face_turn_to(pose, a)
        if best is None or dist < best[2]:
            best = (color, a, dist, face)
    return best
...
async def _place_after_arrival(ctx: Any, memory: AgentMemory) -> dict[str, Any]:
    ...
    result = None
    place_stuck = 0   # place 전 전진이 '연속' 막힌 횟수(상한 도달 시만 중단).
    ...
    for i in range(PAD_PLACE_TRIES):
        ...
        # 아직 배송 안 됨 = place 범위(1.20m) 밖 → '제대로' 다가가서 다음 루프가 바로 place 재시도.
        if held0 is None or not await _target_in_range(ctx, held0, PAD_ARRIVAL_AREA):
            print("  place 미배송 + 목표색 pad 벗어남 -> 중단(상위 nav 재접근)")
            break
        if i < PAD_PLACE_TRIES - 1:
            # ★pad 색블롭을 정면에 맞추고 '실제로 움직이는' 속도로 전진(약한 vx는 램프업에 다 죽어
            #   제자리 → 시뮬 정지, 그걸 장애물로 오판하던 것 수정). 이동 후 루프 상단이 place 재시도.
            blob = await _best_color_blob(ctx, held0, PAD_ARRIVAL_AREA)
            if blob is not None and abs(blob.angle_deg) > CENTER_TOLERANCE_DEG:
                await _turn_by_deg(ctx, -blob.angle_deg)   # pad 정면 정렬(빗나가며 전진 방지)
            p0 = await _get_pose(ctx)
            await move_velocity(ctx, vx=PLACE_CREEP_VX, duration_s=PLACE_CREEP_DUR)
            p1 = await _get_pose(ctx)
            moved = math.hypot(p1["x"] - p0["x"], p1["y"] - p0["y"])
            if moved < STALL_ABS_FLOOR_M:
                place_stuck += 1
                print(f"  place 미배송 + 전진 막힘({moved:.2f}m, {place_stuck}회 연속)")
                if place_stuck >= PLACE_STUCK_LIMIT:
                    print("  전진 반복 막힘 -> 중단(상위 nav가 장애물 없는 각도로 재접근)")
                    break
                # 장애물 모서리를 옆으로 살짝 비켜 재시도(넘어짐은 벽걸림뿐이라 허용).
                side = 1.0 if (blob is not None and blob.angle_deg <= 0) else -1.0
                await _strafe(ctx, side)
            else:
                place_stuck = 0
                print(f"  place 미배송 -> {moved:.2f}m 전진, 재시도({i+2}/{PAD_PLACE_TRIES})")
    after = await get_delivered_count(ctx)
    return {
        "placed": after > before,
...
async def visual_navigate_to_pad(
    ...
    *,
    ...
    no_color_turns = 0     # 목표색 안 보여 회전한 연속 횟수(한 바퀴 넘으면 VLM 방향 1회).
    verify_fails = 0       # 색 후보 도착했으나 VLM 글자 검증 실패 누적(소스 A/오검출).
    best_ema = 0.0         # ★지금까지 본 pad 면적(EMA) 최대 = '가까워진 정도'(진전의 참지표).
    area_ema = 0.0         # pad 면적 EMA(노이즈 평활 — 단발 스파이크로 진전/정체 오판 방지).
    stuck = 0              # '전진했는데 안 가까워진' 연속 횟수(회전만 한 attempt는 안 셈).
    escapes = 0            # 갇힘 강제 탈출 발동 누계.
    committed_side = None  # ★탈출/우회 커밋 방향 — 첫 전진-정체에서 목표 쪽으로 latch, 물리적으로
                           #   그쪽이 막힐 때만 뒤집는다(프레임 angle 노이즈에 안 튐 = 왔다갔다 제거).
    advanced_last = False  # 직전 attempt가 '전진'이었나(회전만이면 stuck 카운트 제외).
    last_fwd_side = 1.0    # 직전 전진의 옆걸음 방향(첫 정체 시 committed_side latch 값).
    harvest_seeks = 0      # 수확 pad 지향 이동 발동 누계(무한 추격 방지).
    detour_side = 1.0
    ...
    for attempt in range(PAD_OUTER_MAX):
        ...
        # 1) 색으로 목표 pad blob 찾기(VLM 0).
        blob = await _pad_color_blob(ctx, held_color)
        if blob is None:
            no_color_turns += 1
            # ★랜덤 시작/멀리 있는 pad: 목표색이 안 보여도 이번 런에 '다른' pad를 수확해 뒀다면,
            #   제자리 스핀을 오래 돌기 전에 가장 가까운 수확 pad 쪽으로 지향 이동한다. 패드는 한
            #   구역에 모여 있으므로(관측 사실; 순서는 안 씀) 그 근처로 가면 목표색 blob이 시야에
            #   들어올 확률이 급증 — 방향 없는 스핀보다 훨씬 빠르다. 도달하면 그 anchor는 NEAR
            #   반경에 들어와 자동 제외되고 다음 anchor/블라인드/VLM 폴백으로 우아하게 강등된다.
            pose = await _get_pose(ctx)
            # ★사용자 #2: 이번 런에 목표 C를 이미 봤다면(entry.anchor 기억) 재시작 시 360 돌지 말고
            #   그 방향으로 곧장 지향한다. 자기가 가던 방향에 pad가 있을 확률이 높다.
            tgt_a = entry.get("anchor") if entry is not None else None
            if (tgt_a and no_color_turns >= 1 and harvest_seeks < PAD_HARVEST_MAX_SEEK):
                tdist, tface = _face_turn_to(pose, tgt_a)
                if tdist > PAD_ANCHOR_NEAR_M:
                    harvest_seeks += 1
                    if verbose:
                        print(f"  [pad {attempt}] 목표색 안 보임 + 기억한 C {tdist:.1f}m/{tface:+.0f}° "
                              f"지향 이동({harvest_seeks}/{PAD_HARVEST_MAX_SEEK})")
                    if abs(tface) > PAD_FACE_TOL_DEG:
                        await _turn_by_deg(ctx, max(-PAD_ANCHOR_MAX_REAIM_DEG,
                                                    min(PAD_ANCHOR_MAX_REAIM_DEG, tface)))
                    await _advance_or_detour(ctx, detour_side, memory=memory, goal_turn_deg=tface, verbose=verbose)
                    no_color_turns = 0
                    continue
            harvest = _nearest_harvest_anchor(memory, held_color, pose)
            if (
                harvest is not None
                and no_color_turns >= PAD_HARVEST_SCAN_TURNS
                and harvest[2] > PAD_ANCHOR_NEAR_M
                and harvest_seeks < PAD_HARVEST_MAX_SEEK
            ):
                seen_color, _a, hdist, hface = harvest
                harvest_seeks += 1
                if verbose:
                    print(f"  [pad {attempt}] 목표색 안 보임 + 수확 pad "
                          f"{DESTINATION_SIGN_RULES.get(seen_color, seen_color)} "
                          f"{hdist:.1f}m/{hface:+.0f}° 지향 이동({harvest_seeks}/{PAD_HARVEST_MAX_SEEK})")
                if abs(hface) > PAD_FACE_TOL_DEG:
                    await _turn_by_deg(ctx, max(-PAD_ANCHOR_MAX_REAIM_DEG,
                                                min(PAD_ANCHOR_MAX_REAIM_DEG, hface)))
                await _advance_or_detour(ctx, detour_side, memory=memory, goal_turn_deg=hface, verbose=verbose)
                no_color_turns = 0
                continue
        ...
        # ★진전 판정 = pad 면적(EMA)이 가까워졌나. 물리 이동 아님(옆 드리프트는 면적 안 키움).
        #   stuck은 '전진을 시도했는데 안 가까워진' 경우만 센다 — 조준 회전만 한 attempt를 세면
        #   angle 노이즈로 회전 반복 시 stuck이 헛차 정상 조준 중 탈출이 오발동한다(적대검증).
        area_ema = float(area) if area_ema <= 0 else PAD_AREA_EMA * area + (1 - PAD_AREA_EMA) * area_ema
        progressed = area_ema > best_ema * PAD_PROGRESS_GROWTH
        if progressed:
            best_ema = area_ema
            stuck = 0
            committed_side = None   # 가까워지는 중 → 탈출 커밋 해제(정상 접근 재개).
        elif advanced_last:
            stuck += 1
            if committed_side is None:      # 첫 '전진-정체'에서 목표 쪽으로 방향 latch(진동창 제거).
                committed_side = last_fwd_side
        advanced_last = False               # 이번 attempt 행동으로 다시 판정(전진 분기에서만 True).

        # 2) 색 후보에 충분히 가까움(크고 중앙) -> VLM 광각 글자 검증(좌/중/우: 옆에 치우친 C도 잡음).
        if area >= PAD_ARRIVAL_AREA and abs(angle) <= CENTER_TOLERANCE_DEG * 1.5:
            ...
            if sighting is not None:
                # ★목표 pad(C) 위치를 이번 런에 기억한다(entry=pad_memory[held_color]). 이러면 nav가
                #   중간에 포기·재시작해도 360 재탐색 대신 기억한 C 쪽으로 곧장 지향한다(사용자 #2).
                await _register_local_sighting(ctx, entry, sighting, verbose=verbose)
            ...
            best_ema = 0.0; area_ema = 0.0   # 다른 후보로 넘어가니 진전 기준 리셋(옛 면적에 안 갇히게).
            committed_side = None
            await _turn_by_deg(ctx, PAD_SEARCH_TURN_DEG)
            await _advance_or_detour(ctx, detour_side, memory=memory, verbose=verbose)
            continue

        # 3) 안 가까워지고 오래 갇힘 -> 커밋 벽따라 강제 탈출(방향 latch). 반복 실패면 nav 포기.
        if stuck >= PAD_STUCK_LIMIT:
            escapes += 1
            if escapes > PAD_MAX_ESCAPES:
                if verbose:
                    print(f"  [pad] {escapes - 1}회 탈출해도 안 가까워짐(area {area} vs best {best_ema:.0f}) "
                          "-> nav 포기(recover가 재배치, 다른 접근각)")
                return False
            if committed_side is None:
                committed_side = 1.0 if angle <= 0 else -1.0   # 처음 갇힐 때 목표 쪽으로 latch.
            if verbose:
                print(f"  [pad {attempt}] {PAD_STUCK_LIMIT}회 안 가까워짐 -> "
                      f"{_side_name(committed_side)} 벽따라 탈출({escapes}/{PAD_MAX_ESCAPES})")
            if not await _lateral_bypass(
                ctx, committed_side, PAD_BYPASS_MAX_CHUNKS, memory=memory, verbose=verbose
            ):
                # 그쪽이 '물리적으로' 막힘(옆걸음 자체가 안 됨) → 그때만 방향 반전(노이즈 아님).
                committed_side = -committed_side
                if not await _lateral_bypass(
                    ctx, committed_side, PAD_BYPASS_MAX_CHUNKS, memory=memory, verbose=verbose
                ):
                    # 양쪽 다 물리적 막힘 → 후진으로 접근각 자체를 바꾼다.
                    if verbose:
                        print(f"  [pad {attempt}] 양쪽 벽따라 막힘 -> 후진(접근각 변경)")
        ...
        if abs(angle) > CENTER_TOLERANCE_DEG:
            ...
            # 전진 막히면 옆걸음: 커밋 방향(있으면) 아니면 blob 쪽. latch로 attempt 경계 진동 차단.
            step_side = committed_side if committed_side is not None else (1.0 if angle <= 0 else -1.0)
            last_fwd_side = step_side
            advanced_last = True     # 이번 attempt는 '전진 시도' → 안 가까워지면 다음 top에서 stuck+1.
            await _advance_or_detour(ctx, step_side, memory=memory, goal_turn_deg=-angle, verbose=verbose)
        continue
...
async def recover_motion(ctx: Any, memory: AgentMemory, reason: str | None = None) -> dict[str, Any]:
    ...
    # 2) 선회로 다른 접근 경로를 찾되, ★pad 배송 실패(navigate_to_pad/place)에서는 크게 돌면 패드에서
    #    등을 돌려 다음 nav가 360 재탐색을 하게 된다(사용자 #2) → pad 사유면 선회를 짧게(≤0.6s).
    #    pick 반복 실패는 다른 큐브가 최근접이 되도록 크게 선회한다(제자리 회전 불가).
    is_pad = bool(reason) and ("pad" in reason or "place" in reason)
    extra = 0.3 * memory.pick_fail_streak if memory.recent_pick_fail else 0.0
    sweep_dur = 0.6 if is_pad else min(0.8 + 0.4 * fails + extra, 2.5)
    await move_velocity(ctx, vx=SWEEP_VX, wz=SWEEP_WZ, duration_s=sweep_dur)
