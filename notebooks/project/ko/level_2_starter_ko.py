...
async def move_velocity(
    ...
    *,
    ...
    tmo = duration_s + 8.0   # ★적응형: 명령 길이+RTT 여유. 고정 20s는 무응답 1건에 최악 41s를 태웠다.
    try:
...
FORWARD_VX = 0.6             # target이 중앙일 때 직진 속도. 0.4→0.6: 워크숍01 검증 데모
...
async def _place_after_arrival(ctx: Any, memory: AgentMemory) -> dict[str, Any]:
    ...
    if after <= before and await get_held_cube_info(ctx) is not None:
        print("  place 후 delivered 미증가 + 아직 파지 중 -> 1회 재시도")
        result = await place_nearest_zone(ctx)
        await asyncio.sleep(0.5)
        after = await get_delivered_count(ctx)
    return {
