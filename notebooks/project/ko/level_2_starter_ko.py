...
async def _advance_or_detour(
    ...
    *,
    ...
    await move_velocity(ctx, vx=-0.15, duration_s=PAD_STALL_BACKUP_S)
...
async def _lateral_bypass(
    ...
    *,
    ...
    expected = _expected_advance_m(FORWARD_VX, PAD_ADVANCE_DUR)
    moved_any = False
    for _ in range(chunks):
        ...
        _bump_stat(memory, "path_len_m", moved)
        moved_any = True
        # ★한 걸음마다 place 검토: 우회로 pad 옆을 '따라 이동'하다 스쳐 지나가는 것 방지.
        if probe is not None and await probe():
            if verbose:
                print("           우회 중 place 후보 감지 -> 우회 조기 중단, 원방위 복귀 후 confirm")
            break
