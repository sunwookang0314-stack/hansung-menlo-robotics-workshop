...
async def _place_after_arrival(ctx: Any, memory: AgentMemory) -> dict[str, Any]:
    ...
    for i in range(PAD_PLACE_TRIES):
        ...
        # 아직 배송 안 됨: place 범위(1.20m) 밖이라 더 다가가야 한다. 단 ★장애물 인식·넘어짐 방지:
        #   그냥 vx로 밀면 앞 장애물에 걸린 채 계속 밀어붙여 넘어진다(라이브 fallen 확정). 실제
        #   이동량을 재서 막혔으면 램밍을 멈추고, 목표색이 안 보이면 중단(상위 nav가 다른 각도로 재접근).
        ...
        if i < PAD_PLACE_TRIES - 1:
            p0 = await _get_pose(ctx)
            await move_velocity(ctx, vx=0.2, duration_s=0.5)   # 살짝 전진(저속=넘어짐 여유)
            p1 = await _get_pose(ctx)
            moved = math.hypot(p1["x"] - p0["x"], p1["y"] - p0["y"])
            if moved < STALL_ABS_FLOOR_M:
                # ★앞이 막힘 → 램밍 금지(넘어짐 방지). 여기선 place 못 하므로 중단하고 상위 nav가
                #   장애물 없는 각도로 재접근하게 한다(place 존은 여러 방향에서 접근 가능).
                print(f"  place 미배송 + 전진 막힘({moved:.2f}m) -> 램밍 금지, 중단(상위 nav 다른 각도 재접근)")
                break
            print(f"  place 미배송 -> {moved:.2f}m 전진 후 재시도({i+2}/{PAD_PLACE_TRIES})")
    after = await get_delivered_count(ctx)
    return {
        "placed": after > before,
