...
async def _place_after_arrival(ctx: Any, memory: AgentMemory) -> dict[str, Any]:
    ...
    if after <= before and await get_held_cube_info(ctx) is not None:
        # ★재시도 전 도착 재게이트: 1차 place/cancel로 자세가 틀어졌으면 지금 최근접 존이
        #   1차와 다른(잘못된) 존일 수 있다. 여전히 목표 pad blob이 근접일 때만 재시도한다.
        held = memory.held_color
        if held is not None and await _target_in_range(ctx, held, PAD_ARRIVAL_AREA):
            print("  place 후 delivered 미증가 + 아직 파지 중 -> 재게이트 통과, 1회 재시도")
            result = await place_nearest_zone(ctx)
            await asyncio.sleep(0.5)
            after = await get_delivered_count(ctx)
        else:
            print("  place 후 미증가 + 재게이트 실패 -> 재시도 없이 반환(잘못된 존 방지)")
    return {
        "placed": after > before,
...
async def visual_navigate_to_pad(
    ...
    *,
    ...
    """VLM signage + 경로 기억으로 destination pad를 국소화·접근합니다(Level 2 합법).
    ...
    nav_start = time.monotonic()   # ★이 배송 nav의 벽시계 예산 기준점.
    ...
    if best and best.get("waypoints"):
        ...
        if await _replay_route(
            ctx, best["waypoints"], memory,
            probe=(lambda: _place_probe(ctx, held_color, entry, memory)), verbose=verbose,
        ):
            ...
            if face_turn is not None:
                if abs(face_turn) > PAD_FACE_TOL_DEG:
                    await _turn_by_deg(ctx, face_turn)
                if await _place_probe(ctx, held_color, entry, memory):   # ★벨트 veto+anchor 게이트 포함
                    if verbose:
                        print(f"  [pad] replay 도착 확인  {await _pose_str(ctx)}")
                    _trace_step(memory, action="arrive", source="route", pose=await _get_pose(ctx))
                    return True
            if verbose:
    ...
    for attempt in range(PAD_OUTER_MAX):
        # ★벽시계 예산 초과 → 이 접근은 포기하고 상위 loop로 반환(recover가 자세를 바꿔 재시도).
        #   한 nav가 라운드를 통째로 먹는 것(라이브: 500s+/배송 0)을 막는 안전장치.
        if time.monotonic() - nav_start > PAD_NAV_BUDGET_S:
            if verbose:
                print(f"  [pad] 예산 {PAD_NAV_BUDGET_S:.0f}s 초과 -> 이 접근 포기(recover 후 재시도)")
            _trace_step(memory, action="nav_budget_exceeded", pose=await _get_pose(ctx))
            return False
        ...
        if face_turn is None:
            ...
            if fails % PAD_FWD_BEFORE_TURN == 0:
                ...
                if verbose:
                    print("           못 찾음 -> 전진(새 시야)")
                side = _choose_detour_side(detour_side, detour_fails, anchor_turn)
                if not await _advance_or_detour(
                    ctx, side, memory=memory, goal_turn_deg=anchor_turn, verbose=verbose
                ):
                    detour_fails[_side_name(side)] += 1
                    detour_side = -side  # 우회로도 막히면 다음엔 반대쪽으로.
                else:
                    confirm_cooldown = max(0, confirm_cooldown - 1)
                    # ★한 걸음마다 place 검토: 후보 참이면 다음 반복을 VLM confirm 티어로 강제.
                    if await _place_probe(ctx, held_color, entry, memory):
                        anchor_streak = PAD_ANCHOR_MAX_REUSE
                        ls_streak = LAST_SEEN_MAX_REUSE
            continue
        ...
        if abs(face_turn) > PAD_FACE_TOL_DEG:
            if not await _turn_by_deg(ctx, face_turn):   # 목표 sign을 정면에 맞춤(폐루프)
                # ★회전 자체가 봉쇄됨(벽 접촉) = 그 자리도 막힌 것 → 왔던 길 후진으로 탈출.
                _record_stall(memory, pose)
                if not await _retreat_along_trace(ctx, memory, verbose=verbose):
                    await move_velocity(ctx, vx=-0.3, duration_s=PAD_STALL_BACKUP_S)
                continue
        ...
        # ★anchor 거리 sanity 게이트(라이브 확정 결함): anchor가 "pad는 아직 멀다"는데 큰 중앙
        #   블롭이 보이면 그건 pad가 아니라 초록 컨베이어/동색 오브젝트다(면적∝1/d²). 이 상태로
        #   confirm VLM(10~33s)을 돌리면 conf 0.00 무한 루프(라이브 pad7/8: d≈4.1m, area 2만).
        #   예외: veto가 상한만큼 쌓였고 anchor가 낡았으면(90s+ 미갱신) 오염 가능성 → confirm 1회 허용.
        if (
            in_range
            and source in ("last_seen", "anchor")   # ★방금 VLM으로 글자를 직접 본 도착(vlm)은 면제
            and anchor_dist is not None             #   — 오염 anchor가 진짜 도착을 90초+ 봉쇄하던 회귀 수정
            and anchor_dist > PAD_PLACE_GATE_M
        ):
            anchor_stale = (
                time.monotonic() - float((anchor or {}).get("t", 0.0)) > PAD_ANCHOR_STALE_S
            )
            if gate_veto_streak >= PAD_GATE_VETO_MAX and anchor_stale:
                gate_veto_streak = 0
                if verbose:
                    print("           blob 게이트 반복+anchor 낡음 -> 예외 confirm 1회 허용")
            else:
                gate_veto_streak += 1
                if verbose:
                    print(
                        f"           blob 도착 기각: anchor d={anchor_dist:.2f}m >"
                        f" {PAD_PLACE_GATE_M}m (동색 오인) -> 접근 계속"
                        f" (veto {gate_veto_streak}/{PAD_GATE_VETO_MAX})"
                    )
                _trace_step(
                    memory, action="gate_veto", source=source, area=blob_area,
                    anchor_dist_m=round(anchor_dist, 2), pose=pose,
                )
                in_range = False
        elif in_range:
            gate_veto_streak = 0
        # ★벨트/소스 오인 차단(초록 배송): held색==벨트색이면 fill·pick지점 반경 검사 추가.
        if in_range and blob is not None:
            suspect = _blob_arrival_suspect(blob, held_color, memory, pose)
            if suspect is not None:
                if verbose:
                    print(f"           blob 도착 기각(벨트 오인 차단): {suspect}")
                _trace_step(memory, action="belt_veto", note=suspect, area=blob_area, pose=pose)
                in_range = False
        # ★confirm 쿨다운: 직전 confirm 실패 후 전진 청크를 소화하기 전엔 재confirm 금지.
        if in_range and confirm_cooldown > 0 and source in ("last_seen", "anchor"):
            if verbose:
                print(f"           confirm 쿨다운 {confirm_cooldown}청크 남음 -> 보류, 전진")
            in_range = False
        if in_range:
            if source in ("last_seen", "anchor"):
                ...
                if confirm is None:
                    ...
                    fails += 1
                    # ★제자리 재confirm 금지: 쿨다운 설정 + 반드시 한 청크 움직여 관점을 바꾼다
                    #   (기존엔 continue만 해서 같은 자리·같은 프레임으로 30s confirm 무한 반복).
                    confirm_cooldown = CONFIRM_COOLDOWN_CHUNKS
                    side = _choose_detour_side(detour_side, detour_fails, anchor_turn)
                    # ★방금 '목표 pad 아님'으로 기각한 대형 blob(벨트일 공산) 정면으로 돌진하지
                    #   않도록 먼저 우회각만큼 꺾고 나서 전진해 관점을 바꾼다(검증 지적: 충돌 위험).
                    await _turn_by_deg(ctx, side * PAD_STALL_DETOUR_DEG)
                ...
                if abs(confirm) > PAD_FACE_TOL_DEG:
                    ...
                    if not await _place_probe(ctx, held_color, entry, memory):   # ★동일 엄격 게이트
                        if verbose:
                            print(
            ...
            _trace_step(memory, action="arrive", source=source, pose=await _get_pose(ctx))
            return True
        if verbose:
        ...
        adv_dur = (
            PAD_ADVANCE_FAR_DUR
            if source == "anchor" and anchor_dist is not None and anchor_dist > PAD_FAR_DIST_M
            else PAD_ADVANCE_DUR
        )   # ★원거리 장청크: 램프업 1회로 2청크 거리(움찔 1회 제거). 근거리는 정지 정밀도 우선.
        if await _advance_or_detour(
            ctx, side, duration_s=adv_dur, memory=memory,
            goal_turn_deg=anchor_turn, verbose=verbose,
        ):
            hard_stall_streak = 0
            confirm_cooldown = max(0, confirm_cooldown - 1)
            # ★한 걸음마다 place 검토(사용자 요구): 후보 참 → 다음 반복을 VLM confirm 티어로 강제.
            if await _place_probe(ctx, held_color, entry, memory):
                anchor_streak = PAD_ANCHOR_MAX_REUSE
                ls_streak = LAST_SEEN_MAX_REUSE
        else:
            ...
            if chunks:
                ...
                bypassed = await _lateral_bypass(
                    ctx, target_side, chunks, memory=memory,
                    probe=(lambda: _place_probe(ctx, held_color, entry, memory)), verbose=verbose,
                )
                hard_stall_streak = 0
                bypass_rounds += 1
                if bypassed:
                    ...
                    # ★직진·우회·측면 우회 전부 실패 → 보장 탈출: 왔던 길로 후진.
                    if not await _retreat_along_trace(ctx, memory, verbose=verbose):
                        await move_velocity(ctx, vx=-0.3, duration_s=PAD_STALL_BACKUP_S)
                    anchor_streak = 0   # 탈출 후 anchor 재조준부터 재개.
